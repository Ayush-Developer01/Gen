...
const getConfig = () => {
    ...
    if (!cfg.aliases)
        cfg.aliases = {};
    if (!cfg.noPrefixRoleId)
        cfg.noPrefixRoleId = null;
    return cfg;
};
...
client.on("messageCreate", async (message) => {
    ...
    if (!hasPrefix && (!noPrefixAllowed || (!KNOWN_COMMANDS.has(rawCommand) && !cfg.aliases[rawCommand])))
        return;
    ...
    if (cmd === "setnoprefix") {
        ...
        if (!role)
            return void message.reply(`Usage: \`${prefix}setnoprefix @role\` | \`remove\` to disable`);
        const c = getConfig();
        c.noPrefixRoleId = role.id;
        writeJson("config.json", c);
    ...
    if (cmd === "setalias") {
        ...
        const valid = (value) => !!value && /^[a-z0-9_-]+$/.test(value);
        ...
        if (check === alias)
            return void message.reply(`❌ Alias loop detected.`);
        c.aliases = aliases;
        writeJson("config.json", c);
    ...
    if (cmd === "removealias") {
        ...
        if (!alias)
            return void message.reply(`Usage: \`${prefix}removealias <alias>\``);
        const c = getConfig();
        if (!c.aliases[alias])
            return void message.reply(`❌ Alias \`${alias}\` does not exist.`);
    ...
    if (cmd === "aliases") {
        ...
        if (entries.length === 0)
            return void message.reply("📋 No aliases configured.");
    ...
    if (cmd === "allowguild") {
        ...
        if (!guildId)
            return void message.reply(`Usage: \`${prefix}allowguild <guildId>\`\nCurrent server ID: \`${message.guild?.id}\``);
        const c = getConfig();
        if (c.allowedGuildIds.includes(guildId))
            return void message.reply(`✅ \`${guildId}\` already allowed.`);
    ...
    if (cmd === "denyguild") {
        const guildId = args[0];
        if (!guildId)
            return void message.reply(`Usage: \`${prefix}denyguild <guildId>\``);
        const c = getConfig();
