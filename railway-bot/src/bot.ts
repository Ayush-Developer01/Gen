...
function emoji(cfg: Config, key: string, fallback: string): string {
...
if (!token) { logger.error("DISCORD_TOKEN not set. Exiting."); process.exit(1); }
...
setInterval(() => {
  void (async () => {
    try {
      ...
      for (const [userId, data] of Object.entries(pending)) {
        ...
        if (age >= TIMEOUT_MS) {
          ...
          catch (err) { logger.error(`applyMissBlacklist failed for ${userId}`, err); }
          continue;
        }
    ...
    } catch (err) { logger.error("Vouch interval error", err); }
  })();
}, 30_000);
...
client.on("messageCreate", async (message) => {
  ...
  if (cmd === "help") {
    ...
    if (admin) {
      embed.addFields(
        { name: "📊 Gen Limits", value: [`🚀 Boost   — 45 gens/day · 5 min cooldown`, `⭐ Premium — 30 gens/day · 7 min cooldown`, `👤 Free    — 20 gens/day · 10 min cooldown`].join("\n") },
  ...
  if (cmd === "checkstatus") {
    ...
    if (currentStatus.toLowerCase().includes(statusText.toLowerCase())) {
      if (member.roles.cache.has(roleId)) {
        ...
        try { await member.roles.add(roleId); await message.reply("✅ Status correct! You have been given the role."); }
        catch { await message.reply("❌ Could not assign the role. Check bot permissions."); }
      }
    } else {
  ...
  if (cmd === "removeemoji") {
    ...
    if (!key || !defaults[key]) return void message.reply(`Usage: \`${prefix}removeemoji <key>\``);
    const c = getConfig();
